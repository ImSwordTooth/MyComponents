<template>
	<div
		:class="['select', { 'withLabel': label }, { 'multi': isMulti }]"
		:style="{ width: `${width}px` }"
		ref="baseElement"
		@mousedown="openOptions"
	>
		<div class="textPart">
			<div :class="['label', {'withValue': isMulti ? value.length > 0 : !!value }]" v-if="label">{{ label }}</div>
			<div class="value" v-if="value">
				<template v-if="isMulti">
					<slot name="multiRender" v-for="(optionValue, index) of value" :option="options.find(o => o.value === optionValue)">
						{{ options.find(o => o.value === optionValue)?.label || options.find(o => o.value === optionValue)?.value }}{{ index < value.length - 1 ? ',' : '' }}
					</slot>
				</template>
				<template v-else>
	...
	<teleport to="body" v-if="!isFirstOpen">
		...
		>
			...
			<VisualList class="optionList" v-if="isVisual" :height="listHeight" :per-height="32" :list="options">
				<template #item="{ itemInfo }">
					<li
						:class="{ 'active': getIsActiveOption(itemInfo.value), 'disabled': itemInfo.disabled }"
						:data-value="itemInfo.value"
						@click="changeValue"
					>
						<slot name="option" :label="itemInfo.label" :value="itemInfo.value" :img="itemInfo.img">
							{{ itemInfo.label || itemInfo.value }}
						</slot>
			...
			<div v-else class="optionList" :style="{ maxHeight: `${listHeight}px` }">
				<template v-for="option of options" :key="option.value">
					<li
						:class="{ 'active': getIsActiveOption(option.value), 'disabled': option.disabled }"
...
const slots = defineSlots<{
	...
	multiRender(option: SelectOptions): never
}>()
...
const changeValue = (e: MouseEvent) => {
	...
	if (props.isMulti) {
		const newList = (value.value as unknown as (string|number)[]).slice()
		const index = newList.findIndex((n: string|number) => n === optionValue)
		if (index > -1) {
...
const getIsActiveOption = (currentValue: string | number) => {
	if(props.isMulti) {
		return (value.value as (string|number)[] ).includes(currentValue)
	} else {
		return (value.value as (string | number) ) === currentValue
	}
}

watch([open, value], getLocation)
